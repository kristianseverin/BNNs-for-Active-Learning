...
import numpy as np
import pandas as pd
from Utils import custom_data_loader_classification, preprocess_activeL_data_classification, preprocess_activeL_data_classification_alldata
...
#model = LargeFFBNNClassificationPaper(4, 5)
...
dataset_train, dataset_test, dataset_activeL, df_custom = preprocess_activeL_data_classification_alldata()
...
class runActiveLearning():
        ...
        if rounds == 1:
            ...
            with torch.no_grad():
                for batch_index, (X, y) in enumerate(self.all_data):
                    ...
                    ensemble_outputs = ensemble_outputs.reshape(self.instances, batch_size, 5)
    ...
    def annotateSelectedData(self, rounds):
        ...
        def determine_quality(data):
            ...
            # make quality of food zero indexed
            data['quality_of_food'] = data['quality_of_food'] - 1
            return data

        def determine_quality_nonoise(data):
            quality_based_on_income = np.where(data['income'] >= 7000, 5,
            np.where(data['income'] >= 4000, 4,
            np.where(data['income'] >= 3000, 3,
            np.where(data['income'] >= 2000, 2, 1))))
            print(f'this is the quality based on income: {quality_based_on_income}')
            quality = quality_based_on_income        

            print(f'this is the quality: {quality}')

            data['quality_of_food'] = quality
            # make quality of food zero indexed
            data['quality_of_food'] = data['quality_of_food'] - 1
            return data


        '''the different oracles should in theory work for both noisy and non-noisy data.
        The noisy data is used to simulate the real world scenario where the data is not perfect, 
        and the oracle is annotating as a reflection of the data generating process. 
        The non-noisy data is used to demonstrate how the active learning process works in a perfect world scenario
        '''
        if self.noisy_data == True:
            annotated_data = determine_quality(df)
        else:
            annotated_data = determine_quality_nonoise(df)
    ...
    def TrainModel(self, rounds, epochs, is_validation):
        ...
        for batch_index, (inputs, targets) in enumerate(self.running_accumulation_annots):
            ...
            # save training loss
        np.save('/Users/kristian/Documents/Skole/9. Semester/Thesis Preparation/Code/BNNs/ThesisPlots/Results/SeedModelNoiseBBB/trainloss' + str(rounds) + '.npy', t_loss)
    ...
    def TestModel(self, rounds):
        ...
        df_loss = pd.DataFrame(test_loss, columns = ['TestLoss'])
            # save the loss
        df_loss.to_csv('ThesisPlots/Results/SeedModelNoiseBBB/TestLoss' + str(rounds) + '.csv', mode ="a" ,index=False)
            # without overwriting
        ...
        df.to_csv('ThesisPlots/Results/SeedModelNoiseBBB/results' + str(rounds) + '.csv', mode='a', index=False)
        ...
        '''
        df = pd.read_csv('ThesisPlots/Results/SeedModelNoiseBBB/results' + str(rounds) + '.csv', names = ['Actual', 'Predictions', 'accuracy', 'precision', 'recall', 'f1_score'], header = 0)
        print(f'This is the dataframe: {df.head()}')
    ...
    def Test_ensemble(self, rounds, is_sample=True):
        ...
        with torch.no_grad():
            for data, target in self.test_loader:
                ...
                outputs = torch.zeros(test_samples, 128, 5).to(device)
        ...
        # save the results
        df.to_csv('ThesisPlots/Results/SeedModelNoiseBBB/ensemble_results' + str(rounds) + '.csv', mode='a', index=True)
        print(df)

        
    def getTrainedModel(self, rounds, epochs):
        # path to save the trained model
        self.train_weight_path = 'trainedModels/trained_weights/SeedModelNoiseBBB/' + self.model_name + '_' + 'e' + str(epochs) + '_' + '-r' + str(rounds) + '-b' + str(self.batch_size) + '.pkl'
...
if __name__ == '__main__':
    if not os.path.isdir('trainedModels/trained_weights/SeedModelNoiseBBB'):
        os.makedirs('trainedModels/trained_weights/SeedModelNoiseBBB')


    # use the class to run the active learning
    active_learning = runActiveLearning(model_name='BBB', model=model, dataloader_train=dataset_train, top_unc = 100, 
    dataloader_test=dataset_test, dataset_active_l= dataset_activeL, epochs=100, rounds=1, learning_rate=0.0001, batch_size=128, instances = 30, seed_sample=1, 
    ...
    for e in range(1, 1):
        ...
        # save the model
        active_learning.saveModel(model, active_learning.optimizer, 'trainedModels/trained_weights/SeedModelNoiseBBB/' + 'BBB' + '_' + 'e' + str(e) + '_' + '-r' + str(1) + '-b' + str(active_learning.batch_size) + '.pkl')
    ...
    for r in range(1, active_learning.rounds+1):
        ...
        for e in range(1,active_learning.epochs+1):
            ...
            active_learning.saveModel(model, active_learning.optimizer, 'trainedModels/trained_weights/SeedModelNoiseBBB/' + 'BBB' + '_' + 'e' + str(e) + '_' + '-r' + str(r) + '-b' + str(active_learning.batch_size) + '.pkl')
