...
class runActiveLearning():
    ...
    def TrainModel(self, rounds, epochs, is_validation):
        ...
        for batch_index, (inputs, targets) in enumerate(self.running_accumulation_annots):
            ...
            # save training loss
        np.save('/Users/kristian/Documents/Skole/9. Semester/Thesis Preparation/Code/BNNs/ThesisPlots/Results/EPICLE/HHC07ALL/trainloss' + str(rounds) + '.npy', t_loss)
    ...
    def TestModel(self, rounds):
        ...
        with torch.no_grad():
            for batch_index, (inputs, targets) in enumerate(self.dataloader_test):
                ...
                predictions_all_instances.append(outputs.detach().cpu().numpy())
                #predictions_all_instances = [np.squeeze(arr) for arr in predictions_all_instances]
                #predictions_all_instances = [np.squeeze(arr) for arr in predictions_all_instances]
                # only the first dimension shape is (4,1) keep only the first dimension
            ...
            # save the loss
            df_loss.to_csv('/Users/kristian/Documents/Skole/9. Semester/Thesis Preparation/Code/BNNs/ThesisPlots/Results/EPICLE/HHC07ALL/TestLoss' + str(rounds) + '.csv', mode ="a" ,index=False)
            # without overwriting
        ...
        # save the predictions_all_instances as lists in the dataframe
        df['Predictions_1st'] = [predictions_all_instances[i][0] for i in range(len(predictions_all_instances))]
        df['Predictions_2nd'] = [predictions_all_instances[i][1] for i in range(len(predictions_all_instances))]
        df['Predictions_3rd'] = [predictions_all_instances[i][2] for i in range(len(predictions_all_instances))]
        df['Predictions_4th'] = [predictions_all_instances[i][3] for i in range(len(predictions_all_instances))]

        # remove the lists in the datframe so only the values are saved
        df['Predictions_1st'] = df['Predictions_1st'].apply(lambda x: x.item())
        df['Predictions_2nd'] = df['Predictions_2nd'].apply(lambda x: x.item())
        df['Predictions_3rd'] = df['Predictions_3rd'].apply(lambda x: x.item())
        df['Predictions_4th'] = df['Predictions_4th'].apply(lambda x: x.item())
        ...
        df.to_csv('/Users/kristian/Documents/Skole/9. Semester/Thesis Preparation/Code/BNNs/ThesisPlots/Results/EPICLE/HHC07ALL/HHC071e100ALL' + str(rounds) + '.csv', mode='a', header=False, index=False)
        
                

    def getTrainedModel(self, rounds, epochs):
        # path to save the trained model
        self.train_weight_path = 'trainedModels/trained_weights/HHC07r1e100ALL/' + self.model_name + '_' + 'e' + str(epochs) + '_' + '-r' + str(rounds) + '-b' + str(self.batch_size) + '.pkl'
...
if __name__ == '__main__':
    if not os.path.isdir('trainedModels/trained_weights/HHC07r1e100ALL'):
        os.makedirs('trainedModels/trained_weights/HHC07r1e100ALL')
    ...
    for e in range(1, 1):
        ...
        # save the model
        active_learning.saveModel(model, active_learning.optimizer, 'trainedModels/trained_weights/HHC07r1e100ALL/' + 'Dense' + '_' + 'e' + str(e) + '_' + '-r' + str(1) + '-b' + str(active_learning.batch_size) + '.pkl')
    ...
    for r in range(1, active_learning.rounds+1):
        ...
        for e in range(1,active_learning.epochs+1):
            ...
            active_learning.saveModel(model, active_learning.optimizer, 'trainedModels/trained_weights/HHC07r1e100ALL/' + 'Dense' + '_' + 'e' + str(e) + '_' + '-r' + str(r) + '-b' + str(active_learning.batch_size) + '.pkl')
